<section class="container-fluid stats contact-for-help job-offer-apply-bar spaced-container padding-section-vertical">
	<div class="container">
		<h2 class="h1"><?php _e( 'Apply now', 'wikia-careers' ); ?></h2>
		<div class="row">
			<div class="col-xxs-1 col-xs-1 col-sm-2 col-md-4 col-lg-3 col-lg-offset-1">
				<a href="<?php echo $parent_link?>" class="go-back icon-arrow-left-white">
					<?php _e( 'Back to job offers', 'wikia-careers' ); ?>
				</a>
			</div>
			<div class="col-xxs-2 col-xs-2 col-sm-2 col-md-4 col-lg-4">
				<button class="btn-wikia-white-white" data-target="#jobApplyModal" data-toggle="modal">
					<?php _e( 'Apply', 'wikia-careers' ); ?>
				</button>
			</div>
		</div>
	</div>
</section>
